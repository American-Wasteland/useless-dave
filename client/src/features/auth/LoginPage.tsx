import { ArrowRight } from 'lucide-react'
import { Navigate } from 'react-router-dom'
import daveEmblem from '/dave-emblem.svg'
import { Button } from '../../components/ui'
import { useAuth } from './AuthContext'

export function LoginPage() {
  const { user, loading, signInWithGoogle } = useAuth()

  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-background">
        <div className="h-10 w-10 animate-spin rounded-full border-4 border-muted border-t-secondary" />
      </div>
    )
  }

  if (user) {
    return <Navigate to="/" replace />
  }

  return (
    <div className="min-h-screen bg-background overflow-hidden relative">
      {/* Fun background elements */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-20 -left-20 w-72 h-72 bg-secondary/10 rounded-full blur-3xl" />
        <div className="absolute top-1/3 -right-32 w-96 h-96 bg-primary/5 rounded-full blur-3xl" />
        <div className="absolute -bottom-20 left-1/3 w-80 h-80 bg-secondary/5 rounded-full blur-3xl" />
      </div>

      <div className="relative min-h-screen flex flex-col items-center justify-center p-6">
        {/* Logo */}
        <div className="mb-8 text-center">
          <img
            src={daveEmblem}
            alt="Useless Dave"
            className="w-24 h-24 md:w-32 md:h-32 mx-auto mb-4 drop-shadow-xl"
          />
          <h1 className="text-5xl md:text-7xl font-black text-foreground tracking-tight mb-2">
            Useless Dave
          </h1>
          <p className="text-lg text-muted-foreground">
            El ERP con IA que creamos también con IA porque los otros no sirven
          </p>
        </div>

        {/* Main content */}
        <div className="w-full max-w-lg">
          {/* Floating sticker badges */}
          <div className="relative mb-8">
            <div className="flex flex-wrap justify-center gap-3">
              <span className="inline-flex items-center gap-1.5 px-4 py-2 bg-secondary text-secondary-foreground rounded-full text-sm font-bold shadow-lg transform -rotate-2 hover:rotate-0 transition-transform">
                🤖 IA que entiende
              </span>
              <span className="inline-flex items-center gap-1.5 px-4 py-2 bg-primary text-primary-foreground rounded-full text-sm font-bold shadow-lg transform rotate-2 hover:rotate-0 transition-transform">
                ⚡ Cero tutoriales
              </span>
              <span className="inline-flex items-center gap-1.5 px-4 py-2 bg-secondary/80 text-secondary-foreground rounded-full text-sm font-bold shadow-lg transform -rotate-1 hover:rotate-0 transition-transform">
                🏢 Multi-negocio
              </span>
              <span className="inline-flex items-center gap-1.5 px-4 py-2 bg-primary/80 text-primary-foreground rounded-full text-sm font-bold shadow-lg transform rotate-3 hover:rotate-0 transition-transform">
                📊 Info clara
              </span>
            </div>
          </div>

          {/* Login card - slightly tilted for fun */}
          <div className="bg-card rounded-3xl shadow-2xl border-2 border-border p-8 transform hover:scale-[1.02] transition-transform">
            <div className="text-center mb-6">
              <h2 className="text-3xl font-black text-card-foreground mb-2">
                ¡Hola! 👋
              </h2>
              <p className="text-muted-foreground">
                Entra para hablar con Dave
              </p>
            </div>

            <Button
              onClick={signInWithGoogle}
              variant="secondary"
              size="lg"
              className="w-full flex items-center justify-center gap-3 text-base font-bold py-6 rounded-xl"
            >
              <svg className="h-6 w-6" viewBox="0 0 24 24" aria-hidden="true">
                <path
                  fill="currentColor"
                  d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
                />
                <path
                  fill="currentColor"
                  d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
                />
                <path
                  fill="currentColor"
                  d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
                />
                <path
                  fill="currentColor"
                  d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
                />
              </svg>
              Entrar con Google
              <ArrowRight className="w-5 h-5" />
            </Button>

            <p className="text-center text-xs text-muted-foreground mt-6">
              Tus datos están seguros 🔒
            </p>
          </div>

          {/* Fun tagline */}
          <p className="text-center text-sm text-muted-foreground mt-8 font-medium">
            ✨ El ERP que no te hace sufrir ✨
          </p>
        </div>
      </div>
    </div>
  )
}
